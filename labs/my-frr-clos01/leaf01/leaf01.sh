...
iface vxlan0 inet manual
    pre-up ip link add name vxlan0 type vxlan id 10000 local 172.30.1.3 dstport 4789 nolearning
    up ip link set dev vxlan0 up

auto br10
iface br10 inet manual
    pre-up ip link add name br10 type bridge
    post-up ip link set dev vxlan0 master br10 addrgenmode none
